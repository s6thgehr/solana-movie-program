...
mod tests {
    ...
    async fn test_add_comment_instruction() {
        ...
        // Create comment
        // Create review PDA
        let comment: String = "Me, too".to_owned();
        let count: u64 = 0;
        let (comment_pda, _bump_seed) =
            Pubkey::find_program_address(&[review_pda.as_ref(), &count.to_be_bytes()], &program_id);

        // Concat data to single buffer
        let mut data_vec_2 = vec![2];
        data_vec_2.append(
            &mut (TryInto::<u32>::try_into(comment.len())
                .unwrap()
                .to_le_bytes())
            .try_into()
            .unwrap(),
        );
        data_vec_2.append(&mut comment.into_bytes());
